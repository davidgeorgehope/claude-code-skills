...
| Name            | Hex       | RGB                | Usage                          |
|-----------------|-----------|--------------------|--------------------------------|
| Elastic Blue    | `#0B64DD` | rgb(11, 100, 221)  | Primary hero/feature backgrounds, title & closing slides |
| White           | `#FFFFFF` | rgb(255, 255, 255) | Default content slide backgrounds |
| Light Grey      | `#F5F7FA` | rgb(245, 247, 250) | Card fills, icon circle backgrounds |
| Developer Blue  | `#101C3F` | rgb(16, 28, 63)    | Card/container **outlines only** — never as a slide background |
...
2. **Developer Blue is for card outlines only.** At 15%, use it exclusively for card/container outline strokes on content slides. NEVER use Developer Blue as a slide background, footer bar, section divider, or decorative element. Do NOT draw lines, stripes, or shapes with Developer Blue. Elastic Blue owns all hero/dark slide moments.
...
5. **Midnight is NOT a structural color.** At 4%, it's the same weight as Pink or Yellow. Do not use it for backgrounds of any kind. It may appear in small typographic or icon accents only.
...
*Slide backgrounds:*
- Title slides: **Solid Elastic Blue** background with White text — no footer bars, no Developer Blue sections
- Closing/CTA slides: **Solid Elastic Blue** background with White text, centered layout
- Content slides: **White** backgrounds with Dark Ink/Ink text (primary). Use Light Grey sparingly for card fills, not as full slide backgrounds
- Section divider slides: **Elastic Blue** background (same as title) — NOT Developer Blue. Developer Blue is never used as a slide background
- Split-layout slides: Elastic Blue left panel (~30-40% width) with White right panel — use for "big idea + details" slides

*Elastic logo placement (REQUIRED):*
- Title and closing slides: Multicolor Elastic logo + "The Search AI Company" tagline at bottom (left-aligned on title, centered on closing)
- Content slides: Small multicolor Elastic wordmark watermark, bottom-right corner

*Teal accent rules (KEY BRAND PATTERN):*
- Use short Light Teal (#48EFCF) horizontal lines as dividers under card headers, column titles, and section labels
- These are thin (2-4px), roughly 60-80px wide — not full-width rules
- This is the single most distinctive and recurring accent pattern in Elastic decks

*Card and container style:*
- Content cards use Developer Blue (#101C3F) rounded-rectangle **outlines** (1-2px stroke), not filled backgrounds
- Card interiors are White; icon circles use Light Grey fills
- Developer Blue appears only as card/container outline strokes on content slides — never as a slide background, footer bar, or decorative shape/line

*Text and accents:*
- Headers/titles on content slides: Dark Ink or Elastic Blue text
...
- ❌ Do NOT draw vertical or horizontal lines/stripes with Developer Blue on any slide
- ❌ Do NOT use Developer Blue as a background on any slide (title, closing, divider, or content)
- ❌ Do NOT create footer bars or header bars in Developer Blue
- ❌ Do NOT draw decorative shapes, geometric patterns, or ornamental elements with Developer Blue
- ❌ Do NOT place a teal accent bar across the full top of a slide — teal rules are short (1-2 inches), placed under specific text elements
- ✅ Developer Blue is ONLY used as thin outline strokes (1-2px) around card/container shapes on white content slides
...
- Hero sections: Elastic Blue with White text (primary). Developer Blue for card outlines and container borders only
- Split layouts: Elastic Blue left panel (~30-40%) with White right panel for "big idea + detail" sections

**Documents (Word/PDF):**
- Headers: Elastic Blue (primary) or Developer Blue (for major section breaks)
...
### Key Slide Patterns for pptxgenjs

**Title/Closing slides:** Solid Elastic Blue background, no footer bars.
```javascript
slide.background = { color: C.elasticBlue };
// Add Elastic logo at bottom (render as SVG → PNG via sharp)
// Title slides: logo bottom-left. Closing slides: logo centered.
```

**Teal accent rules** (use under card headers and column titles):
```javascript
...
**Card outlines** (Developer Blue stroke, not filled):
```javascript
...
**Content slide watermark** (small elastic logo, bottom-right):
```javascript
// Render small elastic wordmark as PNG, place at ~(8.8, 5.15)
slide.addImage({ data: watermarkBase64, x: 8.8, y: 5.15, w: 1.0, h: 0.25 });
```
